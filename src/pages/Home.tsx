...
export default function Home() {
  ...
  const [pendingMelds, setPendingMelds] = useState<string[][]>([]);
  ...
  const selectedCards: Card[] = useMemo(
    () => me.hand.filter((card) => selected.has(card.id)),
    [me.hand, selected],
  );
  ...
  const pendingTotal = useMemo(() => {
    ...
    for (const group of pendingMelds) {
      const cards = me.hand.filter((card) => group.includes(card.id));
  ...
  const appendableMeldIds = useMemo(() => {
    ...
    if (!layoffMode || selectedCards.length !== 1) {
      return ids;
    }
    ...
    return ids;
  }, [layoffMode, selectedCards, state.melds]);
  ...
  const addPendingMeld = () => {
    ...
    setPendingMelds((previous) => [...previous, [...selected]]);
    clearSelection();
  };
  ...
  const doDiscard = () => {
    ...
    if (humanDiscard(cardId)) {
      resetLocalActions();
    }
  };

  const cancelDiscardTake = () => {
    if (humanCancelDiscardTake()) {
      resetLocalActions();
    }
  };
  ...
  const onMeldClick = (meldId: string) => {
    ...
    if (humanLayOff(selectedCards[0].id, meldId)) {
      clearSelection();
    }
  };
  ...
  const hint = () => {
    ...
    if (melds.length === 0 || total < state.openThreshold) {
      anastra.setMessage({
        text:
    ...
    setPendingMelds(
      melds.map((meld) => meld.map((card) => card.id)),
    );
    clearSelection();
    anastra.setMessage({
      text: total + ' puanlık açılış hazır. Eli Aç ile onayla.',
      type: 'success',
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-800 via-emerald-900 to-slate-900 text-white flex flex-col">
      <div className="flex items-center justify-between px-3 py-2 bg-black/30">
        ...
        <div className="flex gap-3 text-xs md:text-sm">
          <span className="text-sky-300">T1: {state.teamScores[0]}</span>
          <span className="text-rose-300">T2: {state.teamScores[1]}</span>
          <span className="text-white/50">Baraj {state.openThreshold}</span>
        </div>
      </div>
            ...
            <div className="flex items-start justify-center gap-3 min-w-0">
              <div className="flex flex-col items-center gap-1 shrink-0">
                <button
                  ...
                  disabled={!isMyTurn || state.phase !== 'draw'}
                  ...
                  <div className="max-w-[58vw] md:max-w-[480px] overflow-x-auto overflow-y-visible pt-5 pb-5">
                    <div className="flex items-end px-3 min-w-max">
                      {state.discard.map((card, index) => {
                        const isTop = index === state.discard.length - 1;
                        const canSelect = me.hasOpened
                          ? state.melds.some((meld) =>
                              canAppendToMeld(meld, card),
                            )
                          : isTop;
                        const canInteract =
                          isMyTurn && state.phase === 'draw' && canSelect;
                        ...
                        return (
                          <button
                            ...
                            title={
                              canInteract
                                ? me.hasOpened
                                  ? 'Bu kartı işle ve bu karttan itibaren yerdeki kartları al'
                                  : 'Rakibin son attığı kartı al'
                                : me.hasOpened
                                  ? 'Bu kart mevcut bir pere işlenemiyor'
                                  : 'El açmadan yalnızca son kart alınabilir'
                            }
                          >
                            <div className="pointer-events-none select-none">
                              <CardView card={card} size="lg" disabled />
                            </div>
      ...
      <div className="bg-black/40 border-t border-white/10 p-2 md:p-3 space-y-2">
        ...
        <div className="text-center text-xs text-white/60">
          {isMyTurn
            ? state.phase === 'draw'
              ? me.hasOpened
                ? 'Sıra sende: Desteden çek veya yerdeki işleyebileceğin bir kartı seç.'
                : 'Sıra sende: Desteden çek veya rakibin son attığı kartı al.'
              : state.requiredDiscardCardId
                ? me.hasOpened
                  ? 'Yerden aldığın ilk kartı mevcut bir pere işle veya yerden almayı iptal et.'
                  : 'Yerden aldığın kartla elini aç veya yerden almayı iptal et.'
                : me.hasOpened
                  ? 'Kartını atabilir veya perlere işleyebilirsin.'
                  : 'El açabilir (' +
                    state.openThreshold +
                    '+) veya bir kart atabilirsin.'
            : 'Sıra: ' + state.players[state.currentSeat].name}
        </div>
            ...
            {pendingMelds.map((group, index) => {
              ...
              return (
                <button
                  key={index}
                  type="button"
                  onClick={() => removePendingMeld(index)}
        ...
        {isMyTurn && state.phase === 'action' && (
          <div className="flex flex-wrap justify-center gap-2">
            {!me.hasOpened && (
              <>
                ...
                <button
                  ...
                  onClick={doOpen}
                  disabled={
                    pendingMelds.length === 0 && !selectedMeldType
                  }
            ...
            <button
              type="button"
              onClick={doDiscard}
              disabled={selected.size !== 1 || canCancelDiscardTake}
